...
data_files = ["train_dials_v2.json", "dev_dials_v2.json", "test_dlc_2.1_tagging.json"]
#--------------------------------
...
def main(args):
    ### Read ontology file
    fp_ont = open(os.path.join(args.data_dir, "ontology-modified_v2.json"), "r")
    ...
    fp_train = open(os.path.join(args.data_dir, "train.tsv"), "w")
    fp_dev = open(os.path.join(args.data_dir, "dev.tsv"), "w")
    fp_test = open(os.path.join(args.data_dir, "test.tsv"), "w")
    ...
    for idx, file_id in enumerate(data_files):
        ...
        for dial_dict in dials:
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                # new
                if idx == 2:
                    fp_out.write('\t' + json.dumps(turn["history_slot_type"]))
                    fp_out.write('\t' + json.dumps(turn["candiate_dict"])) # 数据中的拼写错误
    ...
    with open(os.path.join(args.data_dir, 'ontology-modified.json'), 'w') as outfile:
